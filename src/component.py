...
class Component(ComponentBase):
    ...
    def run(self):
        ...
        in_table = self.get_input_tables_definitions()[0]

        columns = in_table.columns
        final_columns = columns + ["open_ai_output"]

        out_table = self.create_out_table_definition("analyzed_output", columns=final_columns)

        analyze_messages_in_file(in_file_name=in_table.full_path,
